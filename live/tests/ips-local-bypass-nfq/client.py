...
1. The *bypass* flow: start a flow with our bypass marker, then a story, then
   the tripwire text. The flow should be locally bypassed after the marker:
   its packets still pass through Suricata but skip detection, so the tripwire
   text must not alert. The client still verifies the echo, proving traffic
   kept flowing after the bypass.
...
def bypass_flow():
    with socket.create_connection((HOST, PORT), timeout=10) as s:
        ...
        send_and_verify_echo(s, b"TRIPWIRE", "bypass-flow tripwire")

def control_flow():
    with socket.create_connection((HOST, PORT), timeout=10) as s:
        s.settimeout(10)
        send_and_verify_echo(s, STORY, "control-flow payload")
        send_and_verify_echo(s, b"TRIPWIRE", "control-flow tripwire")
