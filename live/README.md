...
- After the client exits, the runner normally stops the server, sends SIGTERM
  to Suricata, waits for Suricata to exit, and tears down the network environment.
  With `shutdown: client`, the client requests shutdown itself (for example,
  through `suricatasc`). The runner waits up to 30 seconds for Suricata to exit
  without sending a signal. A timeout fails the test before cleanup terminates
  Suricata. The default is `shutdown: sigterm`.

- Finally, the runner performs the configured verification checks.

## Failing a Test

Successful live tests require Suricata to start and exit cleanly after shutdown.
A nonzero exit status, including a forced SIGKILL during shutdown, fails the test.

Configuration tests may instead declare an expected startup failure:
...
With a nonzero `exit-code`, Suricata must exit with that exact status before
logging "Engine started". A timeout, signal, successful startup, or different
exit status fails the test. Such tests omit `client` and `server`; `before`
still runs, and post-teardown checks should verify the specific error diagnostic.
The default `exit-code` is 0 and the default `startup-timeout` is 60 seconds.
Normal tests still require a client script.
