...
def emit_server_stream_with_acks(f, server_seq, client_seq, payload, ts):
    max_payload = 1400
    off = 0
    while off < len(payload):
        chunk = payload[off : off + max_payload]
...
def make_pcap(path, name_len, fill_byte=b"A"):
    ...
    with open(path, "wb") as f:
        ...
        write_record(f, tcp_packet(CLIENT_IP, SERVER_IP, CLIENT_PORT, SERVER_PORT, cseq, 0, 0x02), ts)
        cseq += 1
        ts += 0.001
        write_record(f, tcp_packet(SERVER_IP, CLIENT_IP, SERVER_PORT, CLIENT_PORT, sseq, cseq, 0x12), ts)
        sseq += 1
        ts += 0.001
        write_record(f, tcp_packet(CLIENT_IP, SERVER_IP, CLIENT_PORT, SERVER_PORT, cseq, sseq, 0x10), ts)
        ts += 0.001

        for src_ip, dst_ip, src_port, dst_port, data in [
            (SERVER_IP, CLIENT_IP, SERVER_PORT, CLIENT_PORT, b"RFB 003.008\n"),
            (CLIENT_IP, SERVER_IP, CLIENT_PORT, SERVER_PORT, b"RFB 003.008\n"),
            (SERVER_IP, CLIENT_IP, SERVER_PORT, CLIENT_PORT, b"\x01\x01"),
            (CLIENT_IP, SERVER_IP, CLIENT_PORT, SERVER_PORT, b"\x01"),
            (CLIENT_IP, SERVER_IP, CLIENT_PORT, SERVER_PORT, b"\x01"),
        ]:
            if src_ip == CLIENT_IP:
        ...
        sseq, ts = emit_server_stream_with_acks(f, sseq, cseq, rfb_server_init(name_len, fill_byte), ts)
...
def make_failure_pcap(path, reason_len, fill_byte=b"B"):
    ...
    with open(path, "wb") as f:
        ...
        sseq, ts = emit_server_stream_with_acks(f, sseq, cseq, b"\x00" + rfb_failure_reason(reason_len, fill_byte), ts)
