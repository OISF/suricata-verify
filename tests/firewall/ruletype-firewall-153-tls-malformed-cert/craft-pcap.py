...
The phase state must not advance when the certificate message fails
to decode (the certificate data does not exist): client_hello and
server_hello are entered by the valid hellos, the client track reaches
client_cert via the valid client certificate, server_cert is never
entered (the server track stays in server_hello, sid 1532 never
fires). The failure suppresses the phase advance of the certificate
message only: the message bytes are consumed, the rest of the flow is
parsed, and nothing is dropped (the decode failure is not an
app-layer error).
