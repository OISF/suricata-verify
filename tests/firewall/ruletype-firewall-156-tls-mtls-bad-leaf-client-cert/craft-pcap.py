...
event is raised for the bad leaf. The client track is in client_cert
from the hello hand-over on (the phase is entered at the message
start, where sid 1562 fires); the certificate phase data is
incomplete (the leaf is missing), and the incomplete-data condition
is sticky: the following ClientKeyExchange does not complete it, so
the data phase is never established through it (sid 1563 never
