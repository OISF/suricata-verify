#!/usr/bin/env python3
"""
Craft the mTLS bad-leaf client certificate firewall test pcap.
...
(server), then:
  ...
  P6 server->client  TLS handshake record: a CertificateRequest (empty
                     body) - it sets the need-client-cert condition
  P7 client->server  TLS handshake record: a client Certificate
                     message with two certificates: a 10-byte garbage
                     "leaf" (not an ASN.1 SEQUENCE) followed by a valid
                     certificate (the committed client_cert.der)
  P8 client->server  TLS handshake record: an empty ClientKeyExchange
                     (the first handshake message after the failed
                     certificate - the suppression must survive it)
  P9 client->server  TCP FIN

tshark dissects the outer TLS records; P7's first certificate is
...
client chain is still decoded (the client certificate phase data is
filled from the first decodable certificate, logged in the tls log
under the client object) and exactly one INVALID_CERTIFICATE decoder
event is raised for the bad leaf. The certificate phase data is
incomplete (the leaf is missing), so the client_cert phase must not
be entered (sid 1562 never fires) - and the incomplete-data condition
must survive the following ClientKeyExchange: the certificate data
alone must not establish the data phase, so the client track stays in
client_hello and never reaches client_data either (sid 1563 never
...
def certificate_request():
    # the CertificateRequest message body is not parsed by the TLS
    # parser (it only sets the need-client-cert condition) - an empty
    # body is enough
...
def client_key_exchange():
    # the ClientKeyExchange message body is not parsed by the TLS
    # parser - an empty body is enough
...
def main():
    ...
    s_seq += len(creq)
    send(CLIENT_IP, SERVER_IP, CLIENT_PORT, SERVER_PORT, c_seq, s_seq, 0x18, cert)
    c_seq += len(cert)
    ...
    with open(DST, "wb") as f:
        ...
        for p in pkts:
            ...
            f.write(p)
    print("wrote %s: %d packets (CH %d B, cert msg %d B, CKE %d B)"
          % (DST, len(pkts), len(ch), len(cert), len(cke)))
