#!/usr/bin/env python3
"""
Craft the TLS incomplete-chain-after-progress firewall test pcap.
...
(server), then:
  ...
  P7 server->client  TLS handshake record: an empty ServerHelloDone
                     (the suppression must survive it)
  P8 server->client  TLS record with content type 23 (application
                     data): it advances the server track to the data
                     phase - the phase-scoped chain guard goes dead
                     there, while cert_data_incomplete stays sticky.
                     The cert_chain_len trap rule must not match on
                     this packet (mid-walk, below the new progress).
...
A committed partial chain of length 1 with sticky cert_data_incomplete
is the exact state the trap rule must never match. The fix suppresses
the keyword while the certificate data is incomplete, independent of
the current phase; the packet pins below are from the branch head.
...
def main():
    ...
    s_seq += len(shd)
    appdata = b"\x17\x03\x03" + struct.pack(">H", 16) + bytes(range(16))
    send(SERVER_IP, CLIENT_IP, SERVER_PORT, CLIENT_PORT, s_seq, c_seq, 0x18, appdata)
    s_seq += len(appdata)
