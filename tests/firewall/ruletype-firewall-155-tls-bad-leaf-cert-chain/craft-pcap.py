...
bad leaf. The server track is in server_cert from the hello
hand-over on (the phase is entered at the message start, sid 1552
fires there); the certificate phase data is incomplete (the leaf is
missing), and the incomplete-data condition is sticky: the following
ServerHelloDone does not complete it, so the data phase is never
established (sid 1555 never fires). The failure is not an app-layer
error: the message bytes are consumed, the rest of the flow is
parsed, and nothing is dropped.
