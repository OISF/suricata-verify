#!/usr/bin/env python3
"""
Craft the TLS fragmented-hello strict-chain firewall test pcap.

Same session geometry as ruletype-firewall-151: a ClientHello split
across two TLS records (60 + 69 body bytes, the SNI in the second),
but the ruleset is the strict migrated chain (a started accept and a
content SNI rule at the hello state, no plain hello accept): the first
fragment is decided at client_hello with no data to match and falls
on the implicit default policy (drop:flow).
