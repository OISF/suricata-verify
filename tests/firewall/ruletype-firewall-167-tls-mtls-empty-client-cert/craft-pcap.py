#!/usr/bin/env python3
"""
Craft the mTLS empty-client-Certificate firewall test pcap.
...
(server), then:
  ...
  P8  client->server  TLS handshake record: a client Certificate with
                      a zero-length chain: the client honored the
                      request by answering it with no certificate
  P9  client->server  TLS handshake record: an empty ClientKeyExchange
  P10 client->server  TCP FIN

A valid empty client Certificate response to a real CertificateRequest
with an otherwise fully valid mTLS session: the no-certificate
decision branch of the logger gate. The client track stays below its
data phase (there is no certificate to complete it, and its empty
message decodes fully, so nothing is anomalous), the server track
completes, and the tls event publishes once at close with the server
content and no client object. Zero anomalies, no drops.
...
def client_certificate_empty():
    # the valid "no certificate" answer to a CertificateRequest (RFC
    # 5246): the chain decodes fully (zero entries), so
    # cert_data_incomplete does NOT stick, yet no client cert content
    # exists - the logger must wait for close to publish the event
    body = b"\x0b" + struct.pack(">I", 3)[1:] + struct.pack(">I", 0)[1:]
    return rec(body)
...
def main():
    ...
    ccert = client_certificate_empty()
