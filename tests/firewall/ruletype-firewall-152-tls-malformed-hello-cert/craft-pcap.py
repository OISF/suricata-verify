...
record and flags the inner message). A phase is entered when its
message starts, so the failed ClientHello still leaves the client
track in client_hello: the failure blocks the hand-over to
client_cert only (the hello's phase data never existed). The valid
ServerHello completes and hands the server track to server_cert
before any certificate byte; the malformed certificate message is
then evaluated in server_cert, without the data phase becoming
possible (a message that failed to parse has no certificate data).
The decode failures only consume bytes: the flow is not dropped and
later messages that decode cleanly still advance.
