#!/usr/bin/env python3
"""
Craft the mTLS flight-order test pcap.

A mutual-TLS 1.2 session in the standard flight order:
...
The point: the server certificate is parsed before the CertificateRequest
sets the mTLS flag. Under the cross-track data shortcut the server
certificate packet pushed the client track to client_data, the client
certificate message was a monotonic no-op and the client_cert phase was
never established for the client's own certificate. With per-track
writes the client track enters client_cert at the client certificate
message (pkt 8), where the buffer-carrying rule fires.
