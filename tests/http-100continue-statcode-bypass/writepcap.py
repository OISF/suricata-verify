#!/usr/bin/env python
"""
Generate input.pcap for the http-100continue-statcode-bypass test.

Demonstrates the 100-Continue detection deviations (false negative on
the final response's stat_code, false positive double alert on the 100
line) with a STANDARD, spec-compliant exchange - a legitimate client
(RFC 7231 Expect: 100-continue, waiting for the 100 before sending the
body) and a plain server answer (a bare "100 Continue" status line,
then the final 200). The client acks the server's packets, as any real
TCP client does. The test expects the FIXED behaviour (final-200
stat_code rules fire, 100 line alerts once) and is red on a pre-fix
build, where the final-200 rules do not fire and the 100 line
double-alerts.
...
Why the client ACKs matter: the deviation needs the 100 status line to
be committed in its own parse/detect cycle (response progress HEADERS,
C > P, prefilter final run) BEFORE the empty line rewinds the tx
backwards to LINE. In NIDS mode the stream engine processes each
direction when the opposing side acks it (ACK-triggered processing);
the client's normal auto-ACKs of the 100 line and the empty line give
each of them its own parse/detect cycle. Without those ACKs (a client
that never acks the server data) the stream engine defers all toclient
parsing to flow teardown, the 100 line, the empty line and the final
200 are parsed in one pass, and the deviation is masked.
"""
...
add(ether("bb:bb:bb:bb:bb:02", "aa:aa:aa:aa:aa:01", SERVER, A_SRC, DPORT, A_SPORT, "PA",
         s_seq + 1, a_ack, respA))
# client acks the 200 (normal TCP behaviour)
add(ether("aa:aa:aa:aa:aa:01", "bb:bb:bb:bb:bb:02", A_SRC, SERVER, A_SPORT, DPORT, "A",
         a_ack, s_seq + 1 + len(respA)))
...
# client waits for the 100 (spec-legal, used for large uploads)
...
# server: interim 100 Continue (bare status line - the standard shape),
# own packet; the client acks it (auto-ACK)
