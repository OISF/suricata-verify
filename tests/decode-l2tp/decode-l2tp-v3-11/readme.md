# L2TPv3 over UDP tunnel with a 8 byte cookie and a L2 Sublayer and ephemeral UDP ports

* Tests a setup that I have seen from a specific vendor where L2TPv3 runs from userspace on some of their devices
...
sudo ip l2tp add tunnel \
   ...
   udp_sport 49159 udp_dport 1701

sudo ip l2tp add session name svtest \
    tunnel_id 1 session_id 1 peer_session_id 1 \
    cookie deadbeefcafebabe peer_cookie deadbeefcafebabe \
    l2spec_type default
...
sudo ip l2tp add tunnel \
   ...
   udp_sport 1701 udp_dport 49159
