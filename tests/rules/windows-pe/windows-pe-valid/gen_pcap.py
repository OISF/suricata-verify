#!/usr/bin/env python3
"""Generate input.pcap with a valid minimal PE32 file served over HTTP."""
...
def build_pe():
    """Build a valid minimal PE32 file."""
    # DOS Header (64 bytes)
    dos = bytearray(64)
    dos[0:2] = b'MZ'
    ...
    # Pad headers to 512 bytes
    headers = headers.ljust(512, b'\x00')

    # .text section data (512 bytes of 0xCC)
    text_data = b'\xCC' * 512
...
def main():
    pe = build_pe()
    req = http_request("192.168.1.100", "/test.exe")
    resp = http_response(pe, "test.exe")
    pkts = build_http_stream("192.168.1.50", "192.168.1.100", 50001, 80, req, resp)
    wrpcap("input.pcap", pkts)
    print(f"Written input.pcap: valid PE32, {len(pe)} bytes, {len(pkts)} packets")
