#!/usr/bin/env python3
"""Generate input.pcap with a boundary-case PE file (minimum viable PE headers)."""
...
def build_pe():
    """Build a boundary-case PE with minimum viable headers.

    Layout: DOS(64) + PE sig(4) + COFF(20) + OptionalHeader(224) + SectionHeader(40) = 352
    Padded to 512 bytes (FileAlignment).
    """
    # DOS Header (64 bytes)
    dos = bytearray(64)
    dos[0:2] = b'MZ'
    ...
    coff = struct.pack('<HHIIIHH',
        ...
        0,        # NumberOfSymbols
        224,      # SizeOfOptionalHeader (PE32 standard)
        0x0102,   # Characteristics
    )

    # Optional Header PE32 (224 bytes) - minimum viable values
    opt = bytearray(224)
    ...
    struct.pack_into('<I', sec, 20, 512)       # PointerToRawData
    struct.pack_into('<I', sec, 36, 0x60000020)  # Characteristics

    # Assemble: headers exactly 352 bytes, pad to 512
    headers = bytes(dos) + pe_sig + coff + bytes(opt) + bytes(sec)
    assert len(headers) == 352, f"Headers are {len(headers)} bytes, expected 352"
    headers = headers.ljust(512, b'\x00')

    # Minimal .text section (512 bytes of NOPs)
    text_data = b'\x90' * 512
...
def main():
    pe = build_pe()
    req = http_request("192.168.1.100", "/boundary.exe")
    resp = http_response(pe, "boundary.exe")
    pkts = build_http_stream("192.168.1.50", "192.168.1.100", 50001, 80, req, resp)
    wrpcap("input.pcap", pkts)
    print(f"Written input.pcap: boundary PE32, {len(pe)} bytes, {len(pkts)} packets")
