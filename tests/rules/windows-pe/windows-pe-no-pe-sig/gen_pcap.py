#!/usr/bin/env python3
"""Generate input.pcap with a file that has MZ but invalid PE signature."""
...
def build_fake_pe():
    """Build a file with valid MZ header but 'XX\\x00\\x00' instead of 'PE\\x00\\x00'.

    windows_pe should fail since the PE signature is invalid.
    """
    # DOS Header (64 bytes) - valid MZ
    dos = bytearray(64)
    dos[0:2] = b'MZ'
    struct.pack_into('<I', dos, 60, 64)  # e_lfanew = 64

    # Invalid PE Signature (4 bytes)
    pe_sig = b'XX\x00\x00'  # Not "PE\x00\x00"
    ...
    struct.pack_into('<I', opt, 28, 0x1000)
    struct.pack_into('<I', opt, 32, 0x1000)
    struct.pack_into('<I', opt, 36, 512)
    struct.pack_into('<I', opt, 56, 0x2000)
    struct.pack_into('<I', opt, 60, 512)
    struct.pack_into('<H', opt, 68, 3)
    struct.pack_into('<I', opt, 96, 16)
...
def main():
    pe = build_fake_pe()
    req = http_request("192.168.1.100", "/no_pe_sig.exe")
    resp = http_response(pe, "no_pe_sig.exe")
    pkts = build_http_stream("192.168.1.50", "192.168.1.100", 50001, 80, req, resp)
    wrpcap("input.pcap", pkts)
    print(f"Written input.pcap: no-PE-sig file (XX instead of PE), {len(pe)} bytes, {len(pkts)} packets")
