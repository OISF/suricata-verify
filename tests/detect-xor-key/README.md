# Feature 8671: XOR transform with variable keys

Tests for using the `extract <nbytes> <offset>` syntax in the `xor` transform,
...
- **01** — 1-byte key at body offset 0, XOR starts at offset 1
  (`xor:offset 1,extract 1 0`); 1 alert
...
- **03** — 4-byte key at body offset 0, XOR starts at offset 4
  (`xor:offset 4,extract 4 0`); 1 alert
- **06** — Extra whitespace in offset syntax (`offset   1,  extract 1 0`) is
  accepted; 1 alert
...
- **15** — Static hex key (`xor:"42"`) and variable key (`xor:offset 1,extract 1 0`)
...
- **16** — In-place transform aliasing regression: a leading identity transform
  (`xor:"00"`) forces the following variable-key `xor:extract 1 0` to run in
  place, with the key byte inside the decoded region. The key must be copied
  before decoding or the result is corrupted; 1 alert

### Error tests (rule load failures)

- **04** — Variable key with zero nbytes (`extract 0 0`); Suricata exits with code 1
- **05** — Variable key with missing offset argument (`extract 1`); exits with code 1
...
Test **16** has its own PCAP: an HTTP POST whose body has a 1-byte XOR key
(0x42) at offset 0 followed by `"infected"` encoded with that key.
