#!/usr/bin/env python
"""
Generate input.pcap for the http-100continue-responseline-bypass test.

NIDS reproducer for the 100-continue response_line bypass:
the http.response_line engine registers the same LINE response progress as
...
response line - the final 200's response line is never inspected
(FN) and the 100 line's response line double-alerts (FP). The test
...
sibling test http-100continue-statcode-bypass
