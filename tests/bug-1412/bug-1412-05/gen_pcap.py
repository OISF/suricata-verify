...
The URI contains "/data/05/" so byte_extract extracts "05" (decimal 5).
The response status code is "200", so after matching "2", byte_test checks:
byte at offset 0 relative to after "2" is > 5. '0' = 0x30 = 48 > 5 = true -> alert.
"""
...
print("Generated test.pcap for bidirectional URI(toserver) -> stat_code(toclient) test")
