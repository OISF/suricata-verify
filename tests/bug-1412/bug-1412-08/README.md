# bug-1412-08: bidirectional byte_extract + byte_math in http.uri -> http.stat_msg

Vverify that `byte_math` results derived from `byte_extract` values are correctly cached and restored across direction boundaries.

PCAP: `test.pcap` (HTTP GET to `/math/03/02` with a `200 OK` response).
