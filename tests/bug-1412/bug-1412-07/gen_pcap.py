...
Host header value is "len08.example.com" -> byte_extract extracts "08" (decimal 8).
Response status message is "OK" (2 bytes).
After matching "O", position is 1; only 1 byte remains ("K").
isdataat:!host_val,relative -> NOT 8 bytes remaining at pos 1 -> true (only 1 remains) -> alert.
"""
from scapy.all import IP, TCP, Raw, wrpcap


def make_tcp_stream(req, resp, sport=40003, dport=80):
...
print("Generated test.pcap for bidirectional host(toserver) -> stat_msg(toclient) test")
